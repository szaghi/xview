...
contains
   ...
   subroutine initialize(self, file_name, is_binary, is_cell_centered, is_icc, is_sol, is_level_set, &
   ...
   if (is_icc) then
      ...
      if (is_binary) then
         self%tecvarname = self%tecvarname//' icc'
      else
         self%tecvarname = self%tecvarname//' "icc"'
      endif
   endif
   if (is_sol) then
      ...
      if (is_binary) then
         self%tecvarname = trim(self%tecvarname)//' u v w p'
      else
         self%tecvarname = trim(self%tecvarname)//' "u" "v" "w" "p"'
      endif
      if (is_level_set) then
         ...
         if (is_binary) then
            self%tecvarname = trim(self%tecvarname)//' f f0'
         else
            self%tecvarname = trim(self%tecvarname)//' "f" "f0"'
         endif
      endif
      if (is_zeroeq) then
         ...
         if (is_binary) then
            self%tecvarname = trim(self%tecvarname)//' visc'
         else
            self%tecvarname = trim(self%tecvarname)//' "visc"'
         endif
      elseif (is_oneeq) then
         ...
         if (is_binary) then
            self%tecvarname = trim(self%tecvarname)//' visc vitl'
         else
            self%tecvarname = trim(self%tecvarname)//' "visc" "vitl"'
         endif
      elseif (is_twoeq) then
         ...
         if (is_binary) then
            self%tecvarname = trim(self%tecvarname)//' visc ken eps'
         else
            self%tecvarname = trim(self%tecvarname)//' "visc" "ken" "eps"'
         endif
      endif
   ...
   if (is_binary) then
      error = tecini112(tecendrec, self%tecvarname//tecendrec, self%file_name//".plt"//tecendrec, '.'//tecendrec, 0, 0, 1)
   else
     open(newunit=self%file_unit, file=self%file_name//".dat")
   ...
   subroutine save_block_file_tec(self, is_binary, b, blocks_map, grd, is_cell_centered, icc, sol, patches)
   ...
   contains
      ...
      do p=1, size(extents, dim=1)
         associate(ci1=>extents(p, 1 ), ci2=>extents(p, 2 ), &
                   ...
                   nk1=>extents(p, 11), nk2=>extents(p, 12), file_unit=>self%file_unit, nvar=>self%nvar)
            if (present(patches)) then
               zone_name = 'patch_'//trim(strz(patches(patch), nz_pad=3))//     &
                           '-face_'//trim(str(extents(p, 0), no_sign=.true.))// &
                           '-blk_'//trim(strz(blocks_map(2,b), nz_pad=4))//'-grp_'//trim(strz(blocks_map(1,b), nz_pad=3))
            else
               zone_name = 'blk_'//trim(strz(blocks_map(2,b), nz_pad=4))//'-grp_'//trim(strz(blocks_map(1,b), nz_pad=3))
            endif
